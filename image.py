...
class FinalImage:
    ...
    def format_img(self):
        if self.watermark_obj.watermark_type == "Image":
            ...
            watermark_width = int((self.watermark.width * (watermark_height / self.watermark.height))*self.size_factor)
            watermark_image = self.watermark.resize((watermark_width, watermark_height))
            print("this is watermark info",self.watermark.height,watermark_height, self.watermark.width,watermark_width)
        ...
        if self.watermark_obj.watermark_type == "Label":
            ...
            font = ImageFont.truetype("BAUHS93.TTF", font_size)
            ...
            draw.text((position), text=self.watermark_obj.watermark_content, font=font, fill=self.watermark_obj.cur_color)
